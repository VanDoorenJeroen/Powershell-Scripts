...
$PATHFOLDER = "\\desso.int\DFSDesso\ITScripts\Powershell\Hydra"
$FOLDER = "ctwin_backup"
...
$HYDRA | Foreach {
    ...
    if (-Not $connection) {
        ...
        try {
            ...
            $startuploc = $drive.Root + '\Users\' + $username + '\Desktop\'
            #$ctwinbackup = '\\desso.int\DFSDesso\ITScripts\Powershell\Hydra\Size.reg'
            #Get-ChildItem $startuploc -Include * | Remove-Item -Recurse
            Copy-Item $PATHFOLDER -Destination $startuploc -Force
            "$FOLDER gekopieerd"
        }
        Catch {
            "$_ on $terminal"
